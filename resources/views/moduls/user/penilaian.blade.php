...
[               -2px]">
<h2 class="text-left text-2xl text-primary tracking-[-2px] font-bold">Sistem Pendukung Keputusan Pemilihan Pakan Domba</h2>
        </div>
        ...
        <a href="{{ route('penilaian.user') }}" class="bg-primary text-white px-5 py-2 rounded-full text-lg font-medium"><i class='bx bxs-pencil mr-2'></i>Lakukan Penilaian Mandiri</a>
        </div>
                ...
                <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10 mr-16 w-full max-w-full min-w-full">
                    <div class="w-full flex flex-col gap-3 mb-10">
                        ...
                        @if (sizeof($penilaians) == 0)
                            <div class="w-full text-gray-500 flex flex-col items-center justify-center max-w-md text-center mx-auto mt-10">

                                                           <img src="https://static.vecteezy.com/system/resources/previews/034/795/880/original/ai-generated-cute-sheep-cartoon-image-free-png.png" alt="" class="w-48">
                                <h3 class="text-primary font-bold text-5xl mt-2">Oops!</h3>

                                                       ifclass="mt-3">Tidak ada alternatif pakan yang ditambahkan oleh admin, hubungi admin untuk menambahkan alternatif pakan agar dapat dilakukan perankingan!</p>
                            </div>
                        @else
                                ...
                                <div class="w-full grid grid-cols-2 mb-5 gap-3">
                                    @foreach ($penilaians as $penilaian)
                                        <div class="flex items-center ps-4 box rounded-lg bg-white">
                                                                               ...
                                                                               <input id="{{ $penilaian->id }}" 
                                               type="checkbox" value="{{ $penilaian->id }}" name="selected_penilaians[]" class="w-4 h-4 text-blue-600 bg-gray-100">
                                            <label for="{{ $penilaian->id }}" class="w-full py-4 ms-2 text-base font-medium text-black">{{ $penilaian->jenis_pakan }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="sm:flex items
                                       -center justify-start gap-2">
                                    <button type="submit" class="focus:ring-2 focus:ring-offset-2 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-1 bg-primary hover:bg-primary focus:outline-none rounded">
